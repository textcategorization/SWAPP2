...
# Database Model - not using as of now will remove when i clean up file
class Database:
    def __init__(self):
        host = "utc.cir9oge4q4te.us-east-1.rds.amazonaws.com"
        user = "utc"
        password = "admin1234"
        db = "utc"
    ...
    def list_users(self):
        self.cur.execute("SELECT `username`,`password` FROM `user`")
...
def vectorize_text():
    if request.method == 'POST':
        ...
        dl_data_frame = [clust_df.to_html(classes='table table-striped', header="true")]
